...
static void decode_string(unsigned char *buf, int *cmd, arg_struct *arg, char **xstr_buf, int *xstr_len, int *eq_mode, bool *after_end, bool force_new, bool sw_30_94) {
    ...
    if ((byte2 & 0x80) == 0) {
        ...
        *cmd = CMD_XROM;
        ...
        if (byte1 == 0xf1) {
            switch (byte2) {
                case 0xd5: *cmd = CMD_FIX; arg->val.num = 10; break;
                case 0xd6: *cmd = CMD_SCI; arg->val.num = 10; break;
                case 0xd7: *cmd = CMD_ENG; arg->val.num = 10; break;
                case 0xe5: *cmd = CMD_FIX; arg->val.num = 11; break;
                case 0xe6: *cmd = CMD_SCI; arg->val.num = 11; break;
                case 0xe7: *cmd = CMD_ENG; arg->val.num = 11; break;
        ...
        if (flag == 0 || flag == 1) {
            ...
            if (byte2 == 0xc0) {
                ...
                *cmd = CMD_ASGN01;
                arg->type = ARGTYPE_STR;
                goto do_string;
            } else if (byte2 == 0xc2 || byte2 == 0xc3
                    || byte2 == 0xca || byte2 == 0xcb) {
                ...
                *cmd += key - 1;
                arg->type = byte2 == 0xc2 || byte2 == 0xc3
                            ? ARGTYPE_STR : ARGTYPE_IND_STR;
                goto do_string;
            } else if (byte2 == 0xe2 || byte2 == 0xe3) {
                /* KEYG/KEYX suffix */
                if (byte1 != 0xf3)
                    goto xrom_string;
                key = buf[pos++];
                ...
                *cmd += key - 1;
                int suffix = buf[pos++];
                decode_suffix(*cmd, suffix, arg);
            } else if (byte2 == 0x32 || byte2 == 0x33) {
                /* GTOL/XEQL */
                *cmd = byte2 == 0x32 ? CMD_GTOL : CMD_XEQL;
                two_byte_num:
                if (byte1 != 0xf3)
                    goto xrom_string;
                int hi = buf[pos++];
    ...
    while (!done_flag) {
        ...
        if (flag == 0) {
            ...
            if (byte1 == 0x00)
                ...
                decode_suffix(cmd, suffix, &arg);
            } else if (byte1 == 0xaf || byte1 == 0xb0) {
                /* SPARE functions (see HP-41C instruction table */
                goto skip;
            } else if (byte1 >= 0xb1 && byte1 <= 0xbf) {
                /* 2-byte GTO */
                byte2 = fgetc(gfile);
                if (byte2 == EOF)
                    goto done;
                ...
                if (suffix == EOF)
                    goto done;
                cmd = byte1 <= 0xdf ? CMD_GTO : CMD_XEQ;
                suffix &= 0x7f;
                decode_suffix(cmd, suffix, &arg);
                ...
                switch (argtype) {
                    ...
                    case ARG_NUM99: {
                        ...
                        if (tok_end - tok_start == 2 && isdigit(hpbuf[tok_start])
                                                     && isdigit(hpbuf[tok_start + 1])) {
                            if (!ind && (string_required || argtype == ARG_EQN || cmd == CMD_GETMI
                                        || cmd == CMD_PUTMI || cmd == CMD_GETLI || cmd == CMD_PUTLI))
                                goto line_done;
                            arg.type = ind ? ARGTYPE_IND_NUM : ARGTYPE_NUM;
        ...
        if (error == ERR_NO) {
            ...
            if (current_prgm.dir == eq_dir->id) {
                ...
                if (eqd->map != NULL) {
                    int4 pos = eqd->map->lookup(pc2line(pc));
                    if (pos != -1) {
                        eqn_save_error_pos(current_prgm.idx, pos);
                        eqn_display_error(false);
                        ...
                        // generated code.
                        if (unwind_after_eqn_error()) {
                            clear_message();
                            return_to_eqn_edit(error);
                        }
                    }
                }
            }
