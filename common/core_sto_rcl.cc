...
int generic_div(const vartype *px, const vartype *py, int (*completion)(int, vartype *)) {
    if (px->type == TYPE_UNIT && (py->type == TYPE_UNIT || py->type == TYPE_REAL)
            || px->type == TYPE_REAL && py->type == TYPE_UNIT) {
        vartype *dst;
        int error = unit_div(px, py, &dst);
    ...
    if (px->type == TYPE_REAL && py->type == TYPE_REAL) {
        ...
        *dst = new_real(res);
        if (*dst == NULL)
            ...
            return ERR_NONE;
    } else if ((px->type == TYPE_REAL || px->type == TYPE_UNIT)
            && (py->type == TYPE_REAL || py->type == TYPE_UNIT)) {
        return unit_sub(px, py, dst);
...
int generic_rcl(arg_struct *arg, vartype **dst, bool must_be_writable) {
    int err;
    ...
    switch (arg->type) {
        ...
        case ARGTYPE_STR: {
            bool writable;
            *dst = recall_var(arg->val.text, arg->length, &writable);
            if (*dst == NULL)
                return ERR_NONEXISTENT;
            if (must_be_writable && !writable)
...
int generic_sto(arg_struct *arg, char operation) {
    ...
    if (operation != 0 && stack[sp]->type == TYPE_STRING)
        return ERR_ALPHA_DATA_IS_INVALID;
    if (arg->type == ARGTYPE_NUM && stack[sp]->type == TYPE_UNIT)
        return ERR_INVALID_TYPE;
        ...
        case ARGTYPE_STR: {
            if (operation == 0) {
                ...
                if (operation == '*'
                    ...
                    return ERR_RESTRICTED_OPERATION;
                temp_arg = *arg;
                return apply_sto_operation(operation, loc.value(), false);
            }
        }
        default:
            return ERR_INVALID_TYPE;
...
int map_unary(const vartype *src, vartype **dst, mappable_r mr, mappable_c mc, bool do_units) {
    int error;
    switch (src->type) {
        case TYPE_REAL: {
            phloat r;
        ...
        case TYPE_UNIT: {
            if (!do_units)
                return ERR_INTERNAL_ERROR;
            vartype_unit *u = (vartype_unit *) src;
            phloat r;
            error = mr(u->x, &r);
            if (error == ERR_NONE) {
                *dst = new_unit(r, u->text, u->length);
                if (*dst == NULL)
                    return ERR_INSUFFICIENT_MEMORY;
            }
            return error;
