...
bool shell_loadimage(int extra, int dup_first_y, int dup_last_y) {
    ...
    res = skin_init_image(type, 256, pm->cmap, pm->width, pm->height + extra);
    if (res) {
        ptr = pm->pixels;
        if (extra == 0) {
            ...
            for (i = 0; i < dup_first_y; i++) {
                skin_put_pixels(ptr);
                ptr += pm->bytesperline;
            }
            unsigned char *s = ptr;
            for (i = dup_first_y; i < dup_last_y; i++) {
                skin_put_pixels(ptr);
                ptr += pm->bytesperline;
            }
            int h = dup_last_y - dup_first_y;
            unsigned char *p = s;
            for (i = 0; i < extra; i++) {
                if (i % h == 0)
                    p = s;
            ...
            for (i = dup_last_y; i < pm->height; i++) {
                skin_put_pixels(ptr);
                ptr += pm->bytesperline;
            }
        }
